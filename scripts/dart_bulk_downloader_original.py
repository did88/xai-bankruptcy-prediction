...
def filter_kospi_kosdaq_non_financial(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "corp_cls" in df.columns:
        ...
        # Alternative: just use firms with stock codes
        print("Warning: 'corp_cls' column not found. Using all firms with stock codes.")
        mask = df["stock_code"].notna()
    
    non_financial = df["corp_name"].str.contains("금융|은행|보험|증권|캐피탈|투자|자산운용", na=False) == False
